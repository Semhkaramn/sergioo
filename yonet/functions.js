...
function initializeEventHandlers() {
    ...
    if (popupForm) {
        popupForm.addEventListener('submit', handlePopupUpdate);
        console.log('Popup form event listener added');
    }

    // Admin user form (if super admin)
    const adminForm = document.getElementById('adminUserForm');
    if (window.isSuperAdmin && adminForm) {
        adminForm.addEventListener('submit', handleAdminUserSubmit);
    }
...
async function handlePopupUpdate(e) {
    ...
    if (data && data.success) {
        showToast(data.message || 'Popup ayarları kaydedildi');
    } else {
        popup.error(data?.message || 'Bir hata oluştu');
    }
}
